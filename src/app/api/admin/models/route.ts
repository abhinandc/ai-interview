...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("model_registry")
      .select("id,model_key,provider,purpose,edgeadmin_endpoint,is_active,created_at,api_key_last4")
...
export async function POST(request: Request) {
  try {
    ...
    const { model_key, provider, purpose, edgeadmin_endpoint, api_key } = await request.json()
    ...
    const apiKey: string | null = typeof api_key === "string" && api_key.trim() ? api_key.trim() : null

    const { encryptModelApiKey } = await import("@/lib/ai/model-registry-secrets")
    ...
    return NextResponse.json({ error: error.message || "Failed to add model" }, { status: 500 })
  }
}
