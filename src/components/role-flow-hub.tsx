...
import { Bot, Brain, ChartNoAxesCombined, Play, ShieldCheck, Sparkles, Workflow } from "lucide-react"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import AnimatedProgressBar from "@/components/ui/smoothui/animated-progress-bar"
import { cn } from "@/lib/utils"
import { getRoleWidgetTemplate, normalizeRoleWidgetConfig, type RoleWidgetLane } from "@/lib/role-widget-templates"
...
const laneIcons: Array<ComponentType<{ className?: string }>> = [Brain, ShieldCheck, ChartNoAxesCombined, Workflow]
const laneAccents = ["text-primary", "text-chart-2", "text-chart-1", "text-chart-3"]
...
export function RoleFlowHub({
  ...
  const uiLanes = useMemo(() => {
    ...
    const resolved = configured.length > 0 ? configured : getRoleWidgetTemplate(roleFamily)
  ...
  useEffect(() => {
    ...
    const delaySeconds = Math.max(0.35, Math.min(6, parseEtaSeconds(step?.eta)))

    const timeout = setTimeout(() => {
      setRunState((prev) => {
        if (!prev || prev.status !== "running" || prev.laneId !== lane.id) return prev
        const nextStep = prev.stepIndex + 1
        if (nextStep >= lane.steps.length) {
          return { ...prev, stepIndex: lane.steps.length - 1, progress: 100, status: "done" }
        }

        const nextProgress = Math.round((nextStep / Math.max(1, lane.steps.length)) * 100)
        return {
          ...prev,
          stepIndex: nextStep,
  ...
  const runLane = (lane: FlowLaneUI) => {
    ...
    setRunState({ laneId: lane.id, stepIndex: 0, progress: 12, status: "running" })
  ...
  return (
    <div className={cn("space-y-4", className)}>
      {uiLanes.map((lane, laneIndex) => {
        ...
        const isLaneRunning = runState?.laneId === lane.id && runState.status === "running"
        const isLaneDone = runState?.laneId === lane.id && runState.status === "done"
        const activeStepIndex = runState?.laneId === lane.id ? runState.stepIndex : null
        const progressValue = runState?.laneId === lane.id ? runState.progress : 0
        ...
        return (
          ...
          >
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs uppercase tracking-[0.18em] text-muted-foreground">{lane.subtitle}</p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {isLaneRunning ? "Running" : isLaneDone ? "Complete" : "Run"}
                  </Button>
                </div>
              </div>
            </CardHeader>
            ...
            <CardContent className="space-y-3">
              <AnimatedProgressBar
                value={progressValue}
                color={laneIndex === 1 ? "oklch(var(--chart-2))" : laneIndex === 2 ? "oklch(var(--chart-1))" : "oklch(var(--chart-3))"}
                className="[&_div]:!rounded-md"
              />

              <div className="grid gap-2">
                {lane.steps.map((step, stepIndex) => {
                  const isActive = activeStepIndex === stepIndex && isLaneRunning
                  const isComplete = isLaneDone || (runState?.laneId === lane.id && stepIndex < (activeStepIndex ?? 0))
      ...
      <Card className="glass-surface relative border-0 bg-transparent shadow-none">
        ...
        <CardContent className="flex items-center justify-between gap-3 pt-6">
          <div>
            <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Orchestration</p>
            <p className="text-sm font-medium">Role widget flows are configured by hiring manager and stream live to candidate workspace.</p>
          </div>
          <div className="flex items-center gap-2">
